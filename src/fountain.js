...
(function (global) {
  ...
  function selfFrame(payloadBytes, type) {
    // Frame type 1 (v4 bare-peel hardening, ruled 2026-08-30): a CRC16 of the
    // payload rides AFTER it — outside len, so a length-only (v1) reader
    // parses a type-1 frame unchanged and simply never sees the check, while
    // a type-aware reader gets content-borne validation with NO identity
    // needed: the 24-bit mode's headers carry no pcrc, and this is where the
    // liar class (chance-passed CRC8 droplets) finally dies on bare peels.
    var t = (type || 0) & 255;
    var tail = t === 1 ? 2 : 0;
    var out = new Uint8Array(payloadBytes.length + 4 + tail);
    out[0] = (payloadBytes.length >> 8) & 255; out[1] = payloadBytes.length & 255;
    out[2] = t; out[3] = 0;
    out.set(payloadBytes, 4);
    if (t === 1) {
      var pc = crc16(payloadBytes);
      out[4 + payloadBytes.length] = pc >> 8;
      out[5 + payloadBytes.length] = pc & 255;
    }
    return out;
  }
  ...
  function unframe(bytes) {
    ...
    var fr = { len: len, type: bytes[2], bytes: bytes.subarray(4, 4 + len) };
    if (fr.type === 1) {
      // Type 1 carries crc16(payload) after the payload. A frame that claims
      // type 1 but cannot fit or match its check is REFUSED content — crcOk
      // false — never silently degraded to unchecked.
      if (bytes.length < 4 + len + 2) fr.crcOk = false;
      else {
        var want = (bytes[4 + len] << 8) | bytes[5 + len];
        fr.crcOk = crc16(fr.bytes) === want;
      }
    }
    return fr;
  }
  ...
  function encodeCarousels(profile, payloadBytes, opts) {
    ...
    // Frame type defaults to 1 (crc16-tailed) for every new emission — the
    // ruled wire change (2026-08-30). opts.type: 0 pins the legacy frame
    // (era tests, A/B specimens); v1 readers parse type-1 frames unchanged.
    if (framed) payloadBytes = selfFrame(payloadBytes, opts && opts.type !== undefined ? opts.type : 1);
  ...
  function assemble(perRing, profile, opts) {
    ...
    // The alt wins ONLY when it VALIDATES. The old rank preferred an
    // honest-incomplete alt over a completed-but-invalid declared read —
    // reasonable when completed-invalid could only mean "wrong subset rule",
    // but that inference is obsolete now that completed-invalid is the
    // LEASE'S POISON SIGNATURE: the caller's provisional rejection and the
    // liar eliminator both trigger on it, and the alt-incomplete preference
    // silently disarmed both (TH7d's bad leg, caught 2026-08-30 — the
    // poisoned union came back "peel incomplete" and the rejection never
    // fired). A genuinely mis-versioned clip still rescues itself here: at
    // the right version it completes AND validates.
    if (res2.ok === true) {
  ...
  /* LIAR ELIMINATION (bare-peel hardening layer 1, 2026-08-30). When a peel
     COMPLETES and fails validation or framing, the prime suspect is a
     chance-passed CRC8 droplet (the priced 1/256 residual — the a42g
     conviction, and the a42u field take that recovered 168/168 and refused).
     A completed solve over TRUE equations is a linear system with one
     solution: excluding a GOOD droplet reproduces the same bytes, excluding
     a LIAR changes them. So: singles pass (accept the first exclusion whose
     assembly VALIDATES — the ladder's own oracle: content key > header CRC >
     in-frame CRC > frame parse); singles that merely CHANGE the output form
     the suspect set, and pairs/triples search only that handful. Capped —
     on a dry run the original result returns with the receipts attached. */
  function assembleEliminating(perRing, profile, opts) {
    var first = assemble(perRing, profile, opts);
    if (first.ok === true || first.recovered == null || first.recovered !== first.K) return first;
    var flat = [];
    perRing.forEach(function (ring, ri) {
      if (ring && ring.droplets) ring.droplets.forEach(function (d, di) {
        if (!isHeaderSlot(d.c)) flat.push({ ri: ri, di: di, seed: ring.seed, c: d.c });
      });
    });
    var CAP = 900, tried = 0;
    var minus = function (exclude) {
      return perRing.map(function (ring, ri) {
        if (!ring || !ring.droplets) return ring;
        var drop = null;
        exclude.forEach(function (e) { if (e.ri === ri) { (drop = drop || {})[e.di] = 1; } });
        if (!drop) return ring;
        return { seed: ring.seed, droplets: ring.droplets.filter(function (_, di) { return !drop[di]; }) };
      });
    };
    var accept = function (r, exc) {
      if (r.ok !== true) return null;
      r.excluded = exc.map(function (e) { return e.seed + ":" + e.c; });
      r.eliminationTried = tried;
      return r;
    };
    var suspects = [];
    for (var i = 0; i < flat.length && tried < CAP; i++) {
      tried++;
      var r1 = assemble(minus([flat[i]]), profile, opts);
      var a1 = accept(r1, [flat[i]]);
      if (a1) return a1;
      if (r1.recovered === r1.K && r1.bytes && !byteEq(r1.bytes, first.bytes)) suspects.push(flat[i]);
    }
    for (var a2i = 0; a2i < suspects.length; a2i++)
      for (var b2i = a2i + 1; b2i < suspects.length && tried < CAP; b2i++) {
        tried++;
        var r2 = assemble(minus([suspects[a2i], suspects[b2i]]), profile, opts);
        var a2 = accept(r2, [suspects[a2i], suspects[b2i]]);
        if (a2) return a2;
      }
    if (suspects.length <= 12)
      for (var x = 0; x < suspects.length; x++)
        for (var y = x + 1; y < suspects.length; y++)
          for (var z = y + 1; z < suspects.length && tried < CAP; z++) {
            tried++;
            var r3 = assemble(minus([suspects[x], suspects[y], suspects[z]]), profile, opts);
            var a3 = accept(r3, [suspects[x], suspects[y], suspects[z]]);
            if (a3) return a3;
          }
    first.eliminationTried = tried;
    first.suspects = suspects.length || undefined;
    return first;
  }

  function assembleWith(perRing, profile, opts, ver) {
    var g = geom(profile);
    // HEADER MAJORITY (bare-peel hardening, 2026-08-30): first-header-wins
    // let one magic-byte-passing forged header poison K/len/pcrc for the
    // whole peel (the a42g specimen actually produced one, rejected only by
    // the magic byte). Headers recur every HEADER_EVERY slots on every ring,
    // so tally every parsed sighting and take the majority; ties keep the
    // first-seen. headerOutvoted surfaces how many sightings lost.
    ...
    for (var r = 0; r < perRing.length; r++) {
      ...
      for (var i = 0; i < ring.droplets.length; i++) {
        ...
        if (isHeaderSlot(d.c)) {
          ...
          if (h) {
            var hk = h.K + ":" + h.len + ":" + h.pcrc + ":" + (h.pcrcBits || 0);
            if (!hTally[hk]) { hTally[hk] = { h: h, n: 0 }; hOrder.push(hk); }
            hTally[hk].n++;
          }
        } else lt.push({ ringSeed: ring.seed, c: d.c, bytes: d.bytes });
      }
    }
    var header = null, headerVotes = 0, headerTotal = 0;
    for (var ho = 0; ho < hOrder.length; ho++) {
      var cand = hTally[hOrder[ho]];
      headerTotal += cand.n;
      if (cand.n > headerVotes) { headerVotes = cand.n; header = cand.h; }
    }
    if (!header) return { ok: false, reason: "no header droplet yet", got: lt.length };
    var headerOutvoted = headerTotal - headerVotes;
    var K = header.K;
    ...
    else if (expect16 !== null) validatedBy = "content-key-crc16";
    else if (header.pcrc !== null && header.pcrcBits === 16 && wireCrc !== header.pcrc) { ok = false; why = "payload CRC16 mismatch"; }
    else if (header.pcrc !== null && header.pcrcBits === 16) validatedBy = "header-crc16";
    ...
    else if (header.pcrc !== null && header.pcrcBits === 8) validatedBy = "header-crc8";
    var ftype;
    if (ok && profile.carriage && profile.carriage.self_framing) {
      ...
      else if (fr.crcOk === false) { ok = false; why = "in-frame CRC16 mismatch (frame type 1)"; }
      else {
        out = fr.bytes; ftype = fr.type;
        if (fr.crcOk === true && !validatedBy) validatedBy = "frame-crc16";
      }
    }
    return { ok: ok, reason: why, K: K, recovered: recovered, droplets: uniq.length, bytes: out,
             ftype: ftype, validatedBy: ok ? validatedBy : undefined,
             headerVotes: headerVotes, headerOutvoted: headerOutvoted || undefined,
  ...
  var API = { encodeCarousels: encodeCarousels, collect: collect, crcAlign: crcAlign, assemble: assemble,
              assembleEliminating: assembleEliminating,
