...
(function (global) {
  ...
  function scanAnnulus(rRaw, H, annulus, opts) {
    ...
    if (!all) return null;
    var result = { n: n, mseAll: all.sse / n, all: all, best: null };
    ...
    var tryCut = function (coord, order, axName, ci) {
      ...
      var gain = ((ft.sse + fb.sse) / Math.max(1, ft.n + fb.n - 2 * P)) / (all.sse / dofAll);
      return { axis: axName, ci: ci, cut: cut, gain: gain, top: ft, bot: fb, nTop: ft.n, nBot: fb.n };
    };
  ...
  function repairSeries(series, track, annulus, profile, opts) {
    ...
    var stats = { scanned: 0, gated: 0, torn: 0, repaired: 0, invalidated: 0, slipSuspect: 0, cuts: [],
                  reasons: { nopred: 0, sepLo: 0, sepHi: 0, discrim: 0 } };
    ...
    for (var t = 0; t < entries.length; t++) {
      ...
      if (!scan || !scan.best || !(scan.best.gain < gainMax)) continue;
      stats.torn++;
    ...
    for (var f in torn) {
      ...
      if (sep > sepHi) { stats.reasons.sepHi++; invalidate(); continue; } // beyond a one-frame step: repair would be a guess
